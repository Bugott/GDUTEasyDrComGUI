...
#ifdef WIN32
	inet_pton(AF_INET, drcom_config.remote_ip, &remote_addr.sin_addr);
...
HEART_BEAT_START:
	...
	while (1)
	{
		...
		while (1)
		{
			...
			{
				fprintf(stderr, "recv kap1_1 timeout, retry %d!\n", retry_cnt++);
				fflush(stderr);
			}
			else
			{
...
//        memcpy(drcom_config.host_ip, pkt_data+12, 4);
		...
		while (1)
		{
			...
			if ((length = recvfrom(client_sockfd, pkt_data, 1024, 0, \
				(struct sockaddr *) &remote_addr, &sin_size)) == -1)
			{
				fprintf(stderr, "recv kap1_2 timeout, retry %d!\n", retry_cnt++);
				fflush(stderr);
			}
			else
			{
		...
		while (1)
		{
			...
			if ((length = recvfrom(client_sockfd, pkt_data, 1024, 0,\
				(struct sockaddr *) &remote_addr, &sin_size)) == -1)
			{
				fprintf(stderr, "recv kap2_1 timeout, retry %d!\n", retry_cnt++);
				fflush(stderr);
			}
			else
			{
		...
		while (1)
		{
			...
			if ((length = recvfrom(client_sockfd, pkt_data, 1024, 0,\
				(struct sockaddr *) &remote_addr, &sin_size)) == -1)
			{
				fprintf(stderr, "recv kap2_2 timeout, retry %d!\n", retry_cnt++);
				fflush(stderr);
			}
			else
			{
