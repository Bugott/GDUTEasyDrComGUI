...
namespace _1107Core
{
    class Program
    {
        ...
        private class Socketx : Socket
        {
            ...
            public int recv(byte[] data, ref EndPoint remoteEP)
            {
                while (true)
                {
        ...
        private class G
        {
            ...
            public static List<byte> KeepAlivePackageBuilder(byte number, int random, byte[] tail, byte type = 1, bool first = false)
            {
                ...
                if (type == 3)
                {
                    ...
                    Tuple<List<byte>, bool> res = G.GenerateCRC(data.ToArray(), encrypt_mode);
        ...
        private class PPPOEHeartbeat
        {
            ...
            public List<byte> MakeHeartbeat(byte[] sip, byte[] challenge_seed, bool first = false)
            {
                ...
                if (first)
                {
                    ...
                    data.Add(pppoe_flag); // ?ǵ?һ?????? data += '\x00\x62\x00\x14'
                }
                else
                {
                ...
                Tuple<List<byte>, bool> res = G.GenerateCRC(challenge_seed, encrypt_mode);
                List<byte> crc = res.Item1;
                bool foo = res.Item2;
                data.AddRange(crc);
                if (foo == false)
            ...
            public void Send(Socketx s)
            {
                while (true)
                {
                    ...
                    int recvlen = s.recv(buff, ref target);
                    ...
                    try
                    {
                        recvlen = s.recv(buff, ref target);
